...
export default function AnalyticsPage() {
  ...
  useEffect(() => {
    async function loadStats() {
      try {
        const [data, impactData] = await Promise.all([api.getMemberStats(), api.getImpactStats().catch(() => null)]);
        setStats(data);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-8 gap-4">
        ...
        <div className="bg-white rounded-xl p-4 border border-slate-200 shadow-xs">
          <span className="text-[11px] text-slate-400 font-semibold block uppercase">Programs</span>
          <h3 className="text-2xl font-black text-[#B62A35] mt-1">{impact?.totalPrograms || 0}</h3>
          <span className="text-[11px] text-slate-500 font-medium">Published work</span>
        </div>
        <div className="bg-white rounded-xl p-4 border border-slate-200 shadow-xs">
          <span className="text-[11px] text-slate-400 font-semibold block uppercase">Volunteers</span>
          <h3 className="text-2xl font-black text-[#1D3557] mt-1">{impact?.totalVolunteers || 0}</h3>
          <span className="text-[11px] text-slate-500 font-medium">Active community</span>
        </div>
        <div className="bg-white rounded-xl p-4 border border-slate-200 shadow-xs">
          <span className="text-[11px] text-slate-400 font-semibold block uppercase">Resolved Issues</span>
          <h3 className="text-2xl font-black text-emerald-600 mt-1">{impact?.resolvedIssues || 0}</h3>
          <span className="text-[11px] text-slate-500 font-medium">Community response</span>
        </div>
